...
class WebsiteSearchInherit(Website):
    ...
    def autocomplete(self, search_type=None, term=None, order=None, limit=5, max_nb_chars=999, options=None):
        ...
        for record in results_data:
            ...
            for mapped_name, field_meta in mapping.items():
                ...
                if field_meta.get('name') == "price":

                    product = request.env['product.template'].sudo().search([('id', '=', record.get('id'))])
                    if product:
                        if product.bom_ids:
                            price = '<span class="oe_currency_value">' + str(product.bom_price_total) + '</span>\xa0€'
                            value = Markup(price)
...
class ThemePrimeWebsiteSale(WebsiteSale):
    ...
    def _get_search_order(self, order=None):
        """
        Use ascending website_sequence so lower numbers appear first in listings.
        Preserve any explicit non-website_sequence orders coming from UI.
        """
        computed = super()._get_search_order(order)
        # Normalize to string
        computed = (computed or '').strip()
        # If no explicit order or it relies on website_sequence, force ascending

        return 'is_published desc , website_sequence asc'
        # tokens = computed.lower().split(',')
        # new_tokens = []
        # changed = False
        # for tok in tokens:
        #     t = tok.strip()
        #     if t.startswith('website_sequence'):
        #         # Replace any direction with asc
        #         new_tokens.append('website_sequence asc')
        #         changed = True
        #     else:
    ...
    def cart_update(
        ...
        """Override to handle stock validation errors for website orders"""
        try:
            ...
            # Catch stock validation errors and display on cart page
            if 'stuks op voorraad' in str(e) or 'units are available' in str(e):
                order = request.website.sale_get_order()
                # Render cart page with error message
    ...
    def cart_update_json(
        ...
        try:
            ...
            if 'stuks op voorraad' in str(e) or 'units are available' in str(e):
                ...
                error_response = {
                    ...
                    'line_id': False,  # No line created
                    'notification_info': {
                        'warning': str(e),  # This is what gets displayed
                        'message': str(e),
                    },
                }
                
                # Include display fields if display is True
                if display:
                    error_response['cart_ready'] = order._is_cart_ready() if order else False
                    error_response['website_sale.cart_lines'] = request.env['ir.ui.view']._render_template(
                    ...
                    error_response['website_sale.total'] = request.env['ir.ui.view']._render_template(
                        "website_sale.total", {
                            'website_sale_order': order,
                        }
                    ) if order else ''
                
                # Include product info if dr_cart_flow is enabled
        ...
        # Check if there's a warning about stock availability
        if response.get('notification_info', {}).get('warning'):
            warning = response['notification_info']['warning']
            if 'stuks op voorraad' in warning or 'units are available' in warning:
                # Ensure error flag is set and override notification
                response['error'] = True
                response['notification_info']['message'] = warning
                response['notification_info']['type'] = 'danger'
...
class ThemePrimeMainClass(http.Controller):
    ...
    def _get_computed_product_price(self, product, product_data, price_public_visibility, visibility_label, currency_id):
        ...
        display_price = product.sudo().bom_price_total if product.bom_price_total > 0 else product_data['price']
        return {
            ...
            'price': FieldMonetary.value_to_html(display_price, monetary_options) if price_public_visibility else visibility_label,
            'list_price': FieldMonetary.value_to_html(product_data['list_price'], monetary_options) if price_public_visibility else ' ',
            'bom_price_total_value':product.sudo().bom_price_total,
